...
class Client(models.Model):
    ...
    full_name = models.CharField(
        max_length=240, verbose_name="ФИО", help_text="Укажите ФИО"
    )
    comment = models.TextField(
        verbose_name="Комментарий", **NULLABLE, help_text="Напишите комментарий"
    )
...
class Mailing(models.Model):
    ...
    end_datetime = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата и время завершения рассылки", **NULLABLE
    )
    status = models.CharField(
        max_length=25, verbose_name="Статус", choices=STATUS_CHOICES, default=CREATED
    )
...
class MailingAttempt(models.Model):
    ...
    status = models.CharField(
        max_length=25, verbose_name="Статус", choices=STATUS_CHOICE
    )
